...
def aes_single_round(plaintext: bytes, key: bytes, if_last_round: bool) -> bytes:
    ...
    Args:
        plaintext (bytes): 16-byte input block.
        key (bytes): 16-byte encryption key.
    Returns:
        bytes: Result after one AES encryption round.
    """
    assert len(plaintext) == 16, "Plaintext must be 16 bytes (128 bits)."
    ...
    def sub_bytes(state: bytearray):
        """Perform SubBytes operation using AES S-box."""
