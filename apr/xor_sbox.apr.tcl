...
set top_level xor_sbox

set EXCLUDE_CELLS "PCORNER PFILLH PFILLQ PFILL1 FILL1TR FILL2TR FILL4TR FILL8TR FILL16TR FILL32TR FILL64TR"
#############################################################
...
proc run_floorplan { } { 
    ...
    # placeInstance reset_driver0 10 10 R0 -fixed
    # addHaloToBlock 2 2 2 2 reset_driver0
    ...
    # globalNetConnect VSS -type pgpin -pin VSS -singleInstance reset_driver0 
    # globalNetConnect VDD -type pgpin -pin VDD -singleInstance reset_driver0 
    # sroute
    # Save Floorplan
...
proc run_final { } {
    ...
    # Fix Violations, run these steps for iterations until violations are all cleaned
    ...
    # Violation-cleaning re-route option 1
	detailRoute -fix_drc
    
    # Violation-cleaning re-route option 2: a Stronger Violation Fix Method
	editDeleteViolations
	globalDetailRoute
    
    # Check if any violations still exist
	clearDrc
	verifyGeometry -error 1000000000
	verifyConnectivity -type regular -error 1000000 -warning 500000
	verifyProcessAntenna -error 1000000
    ...
    streamOut "$top_level.gds" -mapFile enc2gdsLM.map -libName ${top_level} -structureName $top_level -stripes 1 -mode ALL
    
    # Output Netlist
    saveNetlist -excludeLeafCell ${top_level}.apr.v
    saveNetlist -excludeCellInst ${EXCLUDE_CELLS} -excludeLeafCell -lineLength 10000000 -includePowerGround -includePhysicalInst ${top_level}.apr.physical.v 
